    ...
    <article className="bg-white shadow rounded p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2">
          <img src={`/images/${entry.images[galleryIndex]}`} alt="" className="w-full h-60 object-cover rounded" onError={(e)=>e.currentTarget.style.opacity=0.5} />
          <div className="mt-2 flex gap-2">
            {entry.images.map((img,idx)=>(
              <button key={img} onClick={()=>setGalleryIndex(idx)} className={`w-16 h-12 rounded overflow-hidden border ${galleryIndex===idx ? 'border-eucalyptus' : 'border-gray-200'}`}>
        ...
        <div className="md:col-span-1">
          ...
          <h3 className="font-medium">Reflection <span className="text-xs text-gray-500">(graded)</span></h3>
